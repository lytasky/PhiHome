...
<%   
  ...
  String sql = "select * from zxjdyd order by id desc";    //执行SQL语句
  ...
  <div class="slidingList" id="slidingList9" style="height:30px;float:left;margin-left:921px;display:none;background:#0f0f0f;padding-top:5px;">
   ...
   <div class="gray-border round-border" style="width:840px;height:600px;background-color:#FFF;margin-top:15px;font-size:14px;padding-top:15px;">  
      <div class="" style="height:25px;line-height:1.5;border-bottom:1px dashed #d3d7da;">
      	<div>名称</div><div>主持老师</div><div>时间</div><div>地点</div>
      </div>
      <% 
   if(intPageCount>0){
...
%>
      <div class="" style="height:25px;line-height:1.5;border-bottom:1px dashed #d3d7da;">
      	<div><%=result.getString("title") %></div><div><%=result.getString("hostedTeacher") %></div><div><%=result.getString("hostTime") %></div><div><%=result.getString("hostPlace") %></div>
      </div>
      <%
  result.next();
  i++;
...
%>
      ...
      <div class="" style="line-height:1.5;width:300px;margin:auto;margin-top:15px;">
      	<a href="zxjdyd.jsp?page=1">首页</a>
<%if(intPage>1){
%>
<a href="zxjdyd.jsp?page=<%=intPage-1%>">上一页</a>
<%}%>
<%if(intPage<intPageCount)
{%>
<a href="zxjdyd.jsp?page=<%=intPage+1%>">下一页</a>
<%}%>
<a href="zxjdyd.jsp?page=<%=intPageCount%>">尾 页</a>
